...
func TestScanHistoryEventAcceptsExactPersistedNumericBoundaries(t *testing.T) {
	...
	maximumKind := []any{
		int64(1), int16(workflow.EventOperatorCommandRecorded), now,
		"", "", "", "", "", int64(0), "operator-command-1", (*time.Time)(nil), "", false,
		[]byte(`{"action":1,"actor":"operator-1","reason":"maintenance"}`),
	}
